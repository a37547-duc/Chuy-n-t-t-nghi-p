...
const NewImportedGamingGear =() => {
  ...
  // Lọc sản phẩm thuộc danh mục "Gaming"
  const gamingProducts = products.filter((product) => product.category?.name === "Gaming");
  
  const limitedProducts = gamingProducts.slice(-10).reverse();

  return (
      <div className="px-[10px] w-full mx-auto border-none opacity-100 max-w-[1100px] mb-6">
        <div className="relative min-h-[416px] bg-blue-500 rounded-md">
        ...
        {!loading && !error && (
          <NewImportedProducts data={limitedProducts} itemsPerPage={window.innerWidth > 540 ? 5 : 2} />
        )}
        </div>
      </div>
  );
}
export default NewImportedGamingGear;